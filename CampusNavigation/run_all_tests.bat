...
for /d %%d in ("%TEST_DIR%\small_cases\*") do (
    ...
    :: 重定向 "< command.txt" — 把 command.txt 的内容作为程序的键盘输入（stdin）
    :: 重定向 "> my_answer.txt" — 把程序的屏幕输出保存到 my_answer.txt
    :: 重定向 "2>nul" — 把标准错误输出（stderr，文件描述符=2）丢进空设备（nul = "黑洞"），不显示
    "%EXE%" < command.txt > my_answer.txt 2>nul

    :: fc — File Compare，比较两个文件内容
    :: /w — 忽略空白字符（空格、制表符等），只比较实质性内容
    :: >nul 2>nul — 把 fc 的输出和错误都隐藏掉（我们只需要它的退出码 errorlevel）
    fc /w my_answer.txt answer.txt >nul 2>nul

    :: errorlevel — 上一条命令（fc）的退出码。0 = 文件一致，非0 = 有差异
    :: !errorlevel! — 用 ! 包裹才能在循环内读到实时值（因为 setlocal enabledelayedexpansion）
    if !errorlevel!==0 (
        :: %%~nxd — 扩展语法，只取文件夹名（去掉路径前缀）
        echo [PASS] %%~nxd
        set /a PASS+=1
    ) else (
        echo [FAIL] %%~nxd
...
:: %PASS% — 循环结束后引用变量时，用 % 包裹即可（此时已不在循环体内部）
echo ========================================
